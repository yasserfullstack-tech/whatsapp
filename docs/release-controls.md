...
1. **Path-filtered release gates: the always-on aggregator now exists, but it is not yet a required check (highest-value follow-up).** `Production Infra`, `Backup Recovery` and `Reporting Scale` are real release gates but are path-filtered, so requiring them directly would block every unrelated PR. [`.github/workflows/release-gate.yml`](../.github/workflows/release-gate.yml) now implements the structural fix. It is triggered by `pull_request` to `main` with **no path filter** and:
