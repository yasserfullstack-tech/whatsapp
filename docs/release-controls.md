...
| `.github/workflows/production-infra.yml` | `Production Infra` | `validate` (no `name:`) | `validate` | **NO** | `pull_request` is **`paths:`-filtered** (`apps/web/next.config.ts`, `apps/{web,api,worker}/**`, `packages/**`, `package.json`, `bun.lock`, `docker-compose.production.yml`, `infra/{docker,production}/**`, `.dockerignore`, `.env.{production,staging}.example`, the workflow file itself and `docs/deployment.md`/`docs/production-infrastructure.md`). GitHub leaves the checks of a workflow skipped by path filtering in `Pending`, so requiring it would permanently block every PR that does not touch those paths. The gate is real but must first be converted to the always-run aggregator in section 12. |
...
1. **Path-filtered release gates are not yet enforceable (highest-value follow-up).** `Production Infra`, `Backup Recovery` and `Reporting Scale` are real release gates but are path-filtered, so requiring them today would block every unrelated PR. The structural fix is one always-on workflow, `release-gate.yml`, triggered by `pull_request` to `main` with no path filter:
   - a lightweight `changes` job that detects which path groups changed;
   - the existing validations (copied, or reusable `workflow_call` workflows) guarded by `if: needs.changes.outputs.<group> == 'true'` — a job skipped by a conditional reports `Success`, so unrelated PRs are not blocked (section 5);
   - a final `release-gate` job with `needs:` on all of them and an `if: always()` guard that fails unless every applicable job succeeded — that single stable check name then replaces `validate` in the required list.
   Until that exists, `Production Infra` etc. continue to run **after** merge on `main`, which is exactly the gap issue #91 describes, so it must stay on the tracking issue.
...
5. **`Load Smoke` is red on `main` and is not a merge gate.** Issue #90 owns the fix. If a per-PR load gate is wanted afterwards, implement it through the aggregator pattern in item 1 rather than by requiring `smoke`. If a merge queue is ever enabled, every required workflow must also add the `merge_group` event, or required checks will never report on queued PRs.
