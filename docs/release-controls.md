...
| `require_code_owner_review` | `false` | A root `CODEOWNERS` now exists ([PR #126](https://github.com/yasserfullstack-tech/whatsapp/pull/126)), so code owners are defined — every pattern resolves to `@yasserfullstack-tech`. Still `false`: with a single maintainer GitHub never requests review from the author of the pull request, so enabling it would not yet act as a control (section 12 item 4). |
...
2. **`CodeQL` is reported by two apps.** Required by name, so both the Actions job and the code-scanning default setup must pass. Optional hardening: pin the required check to the GitHub Actions app by adding `"integration_id"` (the app id is visible as `app.id` in the check-runs API; the Actions app is `15368`) once verified in this repository, which prevents name spoofing by a third-party app. Verified 2026-09-22 on a pull request head: both runs are `conclusion=success`, and the app ids are **`15368`** (`github-actions`, the Actions job) and **`57789`** (`github-advanced-security`, the code-scanning default setup). Pinning is therefore a **trade-off, not a pure hardening**: adding `"integration_id": 15368` would make only the Actions result required and would drop the code-scanning result from the required set, so today's "both must pass by name" would become "the Actions job must pass". Recommended only if the Actions job is confirmed to cover everything the default setup covers. Left unapplied.
...
4. **`CODEOWNERS` added (2026-09-21, [PR #126](https://github.com/yasserfullstack-tech/whatsapp/pull/126)); `require_code_owner_review` deliberately still off.** A root `CODEOWNERS` now exists. GitHub reads only the first file it finds, in the order `CODEOWNERS` → `.github/CODEOWNERS` → `docs/CODEOWNERS`, so this repository uses the root file and a second copy must not be added. There is exactly one maintainer, `@yasserfullstack-tech`, so every pattern resolves to the same account: the explicit entries are not multi-team routing, they make ownership of the sensitive areas visible in review and keep the mapping correct if a second maintainer is ever added.
   ...
   - Section 9's table row for `require_code_owner_review` has been updated to reflect that `CODEOWNERS` now exists; the value it records, `false`, is unchanged.
