...
| Migrator exposure | `whatsapp-migrator` runs as root: the `migrator` stage in `infra/docker/api.Dockerfile` declares no `USER` instruction and the `migrate` Compose service sets no `user:`. Its exposure is therefore higher than the API and worker images, and the non-root argument above does not apply to it. It is still confined by the `ops` Compose profile, `no-new-privileges: true`, one-shot execution with no published ports and no interactive shell, and an immutable digest-pinned image. Making it non-root needs a verified runtime change because migrations write to mounted volumes; that is tracked separately rather than assumed here. |
| Compensating controls | Non-root `USER bun` for the API and worker images, `no-new-privileges` in `docker-compose.production.yml`, immutable digest-pinned release images, and no interactive shell in the runtime path. The migrator relies on profile confinement and one-shot execution instead of a non-root user, as described in the migrator exposure row. |
