...
type Props = {
  ...
  initialAction?: ContactAction | null;
  filterQuery?: string;
  filterStatus?: string;
};
...
export function ContactSuppressionManager({
  contacts,
  events,
  canSuppress,
  canResubscribe,
  ...
  function openSuppress(contact: ContactRow) {
    setMessage(null);
    setAction({ mode: "suppress", contactId: contact.id });
  }
  ...
  return <div className="contactManagementGrid"><section>
    ...
    {selected && mode === "suppress" ? <form className="consentActionForm" onSubmit={submitSuppress}>
      ...
      <div className="formActions"><button className="primary" disabled={busy} type="submit">{busy ? messages.common.saving : messages.ui.suppressContact}</button><button className="secondary" disabled={busy} onClick={close} type="button">{messages.common.cancel}</button></div>
    </form> : null}
    {selected && mode === "resubscribe" ? <form className="consentActionForm" onSubmit={submitResubscribe}>
      ...
      <div className="formActions"><button className="primary" disabled={busy} type="submit">{busy ? messages.common.saving : messages.ui.restoreEligibility}</button><button className="secondary" disabled={busy} onClick={close} type="button">{messages.common.cancel}</button></div>
    </form> : null}
    <div className="contactRows">{rows.length ? rows.map((contact) => {
      ...
      return <article className="contactRow" key={contact.id}>
        ...
        <div className="contactActions">
          ...
          {canResubscribe && !eligible ? <a className="textButton" href={resubscribeHref} role="button">{messages.ui.recordNewConsent}</a> : null}
        </div>
      </article>;
