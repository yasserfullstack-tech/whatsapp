...
export function ContactSuppressionManager({ contacts, events, canSuppress, canResubscribe }: Props) {
  ...
  async function submitSuppress(event: FormEvent<HTMLFormElement>) {
    ...
    try { const response = await fetch(`/api/contacts/${selected.id}/suppress`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ reason: String(form.get("reason") ?? "") }) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error ?? messages.ui.suppressFailed); setMessage(format(messages.ui.suppressedSuccess, { phone: selected.phoneE164 })); setMode(null); setSelected(null); router.refresh(); }
  ...
  async function submitResubscribe(event: FormEvent<HTMLFormElement>) {
    ...
    try { const response = await fetch(`/api/contacts/${selected.id}/resubscribe`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ consentSource: String(form.get("consentSource") ?? ""), consentedAt: consentedAt.toISOString(), evidenceNote: String(form.get("evidenceNote") ?? ""), confirmation: form.get("confirmation") === "on" }) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error ?? messages.ui.restoreFailed); setMessage(format(messages.ui.restoredSuccess, { phone: selected.phoneE164 })); setMode(null); setSelected(null); router.refresh(); }
