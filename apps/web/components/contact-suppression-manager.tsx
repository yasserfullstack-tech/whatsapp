...
type ContactRow = {
  id: string; phoneE164: string; displayName: string | null; optedIn: boolean;
  optInSource: string | null; optInAt: string | null; unsubscribedAt: string | null;
  suppressedAt: string | null; suppressionReason: string | null; suppressionSource: string | null;
};
...
type Props = {
  contacts: ContactRow[]; events: ConsentEvent[]; canSuppress: boolean; canResubscribe: boolean;
  initialAction?: ContactAction | null; filterQuery?: string; filterStatus?: string;
};
...
export function ContactSuppressionManager({ contacts, events, canSuppress, canResubscribe, initialAction = null }: Props) {
  ...
  const mode: ActionMode | null = action?.mode === "suppress" && selected && canSuppress && !selected.suppressedAt
    ? "suppress"
    : action?.mode === "resubscribe" && selected && canResubscribe && !selectedEligible ? "resubscribe" : null;
  ...
  async function submitSuppress(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setRows((current) => current.map((contact) => contact.id === selected.id ? {
        ...contact, optedIn: false, unsubscribedAt: suppressedAt, suppressedAt,
        suppressionReason: reason, suppressionSource: "dashboard_manual",
      } : contact));
      setMessage(format(messages.ui.suppressedSuccess, { phone: selected.phoneE164 }));
      setAction(null);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : messages.ui.suppressFailed);
  ...
  async function submitResubscribe(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`/api/contacts/${selected.id}/resubscribe`, {
        ...
        body: JSON.stringify({
          consentSource, consentedAt: consentedAt.toISOString(),
          evidenceNote: String(form.get("evidenceNote") ?? ""), confirmation: form.get("confirmation") === "on",
        }),
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error ?? messages.ui.restoreFailed);
      setRows((current) => current.map((contact) => contact.id === selected.id ? {
        ...contact, optedIn: true, optInSource: consentSource, optInAt: consentedAt.toISOString(),
        unsubscribedAt: null, suppressedAt: null, suppressionReason: null, suppressionSource: null,
      } : contact));
  ...
  return <div className="contactManagementGrid"><section>
    ...
    {selected && mode === "suppress" ? <form className="consentActionForm" onSubmit={submitSuppress}>
      <div><p className="eyebrow">{messages.ui.manualSuppression}</p><h3>{selected.displayName ?? selected.phoneE164}</h3><p className="subtitle">{messages.ui.suppressionDescription}</p></div>
      <label><span>{messages.ui.reason}</span><textarea name="reason" defaultValue={messages.ui.defaultSuppressionReason} minLength={3} maxLength={240} required /></label>
      <div className="formActions"><button className="primary" disabled={busy} type="submit">{busy ? messages.common.saving : messages.ui.suppressContact}</button><button className="secondary" disabled={busy} onClick={() => setAction(null)} type="button">{messages.common.cancel}</button></div>
    </form> : null}
    {selected && mode === "resubscribe" ? <form className="consentActionForm" onSubmit={submitResubscribe}>
      <div><p className="eyebrow">{messages.ui.newConsentRequired}</p><h3>{selected.displayName ?? selected.phoneE164}</h3><p className="subtitle">{messages.ui.resubscribeDescription}</p></div>
      ...
      <div className="formActions"><button className="primary" disabled={busy} type="submit">{busy ? messages.common.saving : messages.ui.restoreEligibility}</button><button className="secondary" disabled={busy} onClick={() => setAction(null)} type="button">{messages.common.cancel}</button></div>
    </form> : null}
    <div className="contactRows">{rows.length ? rows.map((contact) => {
      ...
      return <article className="contactRow" key={contact.id}>
        ...
        <div className="contactActions">
          ...
          {canResubscribe && !eligible ? <details className="contactResubscribeDisclosure"><summary className="textButton" role="button">{messages.ui.recordNewConsent}</summary><ContactResubscribeForm contactId={contact.id} phoneE164={contact.phoneE164} displayName={contact.displayName} /></details> : null}
        </div>
      </article>;
