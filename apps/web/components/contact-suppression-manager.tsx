...
export function ContactSuppressionManager({ contacts, events, canSuppress, canResubscribe }: Props) {
  ...
  async function submitSuppress(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`/api/contacts/${selected.id}/suppress`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ reason: String(form.get("reason") ?? "") }) });
  ...
  async function submitResubscribe(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`/api/contacts/${selected.id}/resubscribe`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ consentSource: String(form.get("consentSource") ?? ""), consentedAt: consentedAt.toISOString(), evidenceNote: String(form.get("evidenceNote") ?? ""), confirmation: form.get("confirmation") === "on" }) });
      const payload = await response.json().catch(() => ({}));
