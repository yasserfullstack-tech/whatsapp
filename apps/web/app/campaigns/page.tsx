...
export default async function CampaignsPage() {
  ...
  const [listCounts, segmentCounts] = await Promise.all([Promise.all(lists.map((list) => countEligibleAudience(organizationId, { type: "list", listId: list.id }))), Promise.all(segments.map((segment) => countEligibleAudience(organizationId, { type: "segment", match: segment.match, filters: segment.filters })))]);
  const audiences = [
