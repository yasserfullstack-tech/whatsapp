    ...
    <main className="shell">
      <aside className="sidebar">
        ...
        <nav className="nav" aria-label="Primary navigation">
          {nav.map((item, index) => (
            <Link className={index === 0 ? "navItem active" : "navItem"} href="/dashboard" key={item}>
      ...
      <section className="content">
        ...
        <section className="connectionCard">
          ...
          <div className="connectionCopy">
            <div className="rowTitle">
              <h2>{connected ? "WhatsApp Business connected" : "Connect WhatsApp Business"}</h2>
            ...
            {connected ? (
              <p>
                {connected.verifiedName ?? "WhatsApp Business"} · {connected.displayPhoneNumber ?? connected.phoneNumberId}
                {connected.qualityRating ? ` · Quality ${connected.qualityRating}` : ""}
              </p>
            ) : (
              <p>
                The client signs into Meta in a popup and selects their business, WABA, and phone number. No API tokens or IDs need to be copied into this app.
              </p>
            )}
          </div>
        ...
        <section className="mainGrid">
          <article className="panel campaignsPanel">
            <div className="panelHeader">
              <div>
                <p className="eyebrow">WhatsApp setup</p>
                <h2>{phoneNumbers.length ? "Connected numbers" : "No number connected yet"}</h2>
              </div>
            </div>
            ...
            {phoneNumbers.length ? (
              <div className="numberList">
                {phoneNumbers.map((phone) => (
                  <div className="numberRow" key={phone.id}>
                    <div>
                    ...
                    <div className="numberMeta">
                      <span>{phone.throughputMps} msg/s</span>
                      <span className={phone.status === "connected" ? "status connected" : "status"}>{phone.status}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="emptyState">
                <div className="emptyIcon">W</div>
                <h3>Connect the first client number</h3>
                <p>Meta remains the owner/identity layer. After Embedded Signup finishes, normal work stays inside this dashboard.</p>
              </div>
            )}
          </article>
